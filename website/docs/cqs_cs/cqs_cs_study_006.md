...
![QueryとCommandの分離](./picture/cqs_cs_study_006_pure_query_separation.png)
...
![キャッシュデコレーター](./picture/cqs_cs_study_006_cache_decorator.png)
...
```mermaid
flowchart LR
    Test[テストコード] -->|Query実行| SUT[TodoQueries]
    SUT -->|読み取り| DB[ITodoRepository]
    Test -->|検証| DB
    DB -.->|Verify: Times.Never| Save[SaveAsync]
```
