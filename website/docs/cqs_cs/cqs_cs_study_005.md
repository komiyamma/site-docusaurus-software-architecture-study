...
flowchart TD
  A[Commandの設計] --> B{何を返す？}
  B -->|何も不要| C[パターンA🔵<br/>void/Task]
  B -->|作ったIDが必要| D[パターンB🎪<br/>Guid/long]
  B -->|結果+失敗情報| E[パターンC🎁<br/>Result<T>]
  
  C --> C1[完了しただけ<br/>説明でOK]
  D --> D1[詳細画面へ<br/>遷移に使う]
  E --> E1[失敗を<br/>仕様として扱う]
...
![Return Patterns](./picture/cqs_cs_study_005_return_patterns.png)
...
```mermaid
graph LR
    subgraph AppUnit ["アプリ層 (Inner)"]
        Handler[Command Handler]
        Result[Result / ID]
    end
    subgraph WebUnit ["API層 (Outer)"]
        Endpoint[Minimal API Endpoint]
        HTTP["201 Created / 400 BadReq"]
    end
    
    Handler -->|最小情報を返す| Result
    Result -->|HTTPに変換| Endpoint
    Endpoint -->|レスポンス| HTTP
```
