...
![Ghost Side Effect](./picture/cqs_cs_study_004_ghost_effect.png)
...
sequenceDiagram
    ...
    participant Service as GetTodo()
    participant DB as データベース
    
    App->>Service: 取得したいな🔍
    Service->>DB: データを読み取る
    Note over Service: ついでに閲覧数を+1🔧
    Service->>DB: 更新を保存💥
    Service-->>App: はい、どうぞ（Todo）
    Note over App: あれ？閲覧数が増えてる！？😇
```
...
![Tangled Return](./picture/cqs_cs_study_004_tangled_return.png)
...
sequenceDiagram
    participant App as 呼び出し側
    participant Q as TodoQueries
    participant C as TodoCommands
    participant DB as データベース

    rect rgb(230, 247, 255)
    Note over App, DB: 1. 読むだけ（安全✅）
    App->>Q: 表示して🔍
    Q->>DB: 読み取る
    Q-->>App: TodoDetail
    end
    ...
    Note over App, DB: 2. 必要なら変える（明示的🔧）
    App->>C: 見たよ！と記録して
    C->>DB: 閲覧数を更新して保存
    C-->>App: void/Task
    end
