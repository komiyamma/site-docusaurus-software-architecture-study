...
![Judgement Flow](./picture/cqs_cs_study_003_judgement_flow.png)
...
flowchart TD
  A[メソッド] --> B{名前は？}
  B -->|Get/Find/Search| C[中身を確認👀]
  B -->|Add/Update/Delete| D[Command確定🔧]
  
  C --> E{更新してる？}
  E -->|はい| F[Command💥<br/>名前が偽装]
  E -->|いいえ| G[Query✅<br/>安全]
...
mindmap
  root((CQSの命名慣習))
    Command担当
      作成: Create / Add
      更新: Update / Change / Set / MarkAs
      削除: Delete / Remove
      業務アクション: Complete / Cancel / Pay
    Query担当
      単一取得: Get / Find
      一覧/複数: List / Search
      存在確認: Exists / Any
