...
![Error Sorting](./picture/cqs_cs_study_009_error_sorting.png)

```mermaid
flowchart TD
    Error[エラー/失敗が発生] --> Q{予測できる？<br/>仕様の範囲内？}
    Q -->|YES| Spec[仕様どおりの失敗✅]
    Q -->|NO| Accident[予期しない障害💥]
    
    Spec --> HandleSpec[値を返す/丁寧に通知<br/>例: Result型]
    Accident --> HandleAccident[例外を投げる/ログ出力<br/>例: Exception]
    
    style Spec fill:#E6F7FF
    style Accident fill:#FFE6E6
```
