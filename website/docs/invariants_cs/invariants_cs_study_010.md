...
```mermaid
flowchart TD
  Req[外からの raw 文字列] --> Create[静的 Create メソッド]
  subgraph Inside [Factory 内部]
    Create --> Valid{検証/正規化}
    Valid -- OK --> New[private ctor]
    Valid -- NG --> Fail[Result.Fail]
  end
  New --> Instance[Email インスタンス]
```
...
```mermaid
flowchart LR
  Raw[raw string] -- Create --> Res{Result}
  Res -- Success --> Domain["内部ロジック <br/> (Email型を使用)"]
  Res -- Failure --> UI[エラー表示]
```
