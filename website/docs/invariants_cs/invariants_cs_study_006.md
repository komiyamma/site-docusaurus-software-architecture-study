...
flowchart TD
  ...
  C -- 伝播 --> D[さらに上位...]
  D --> E{catch してる?}
  E -- No --> Crash[アプリ停止/500]
  E -- Yes --> Handle[例外処理/リカバリ]
```
...
```mermaid
flowchart LR
  subgraph Inside [ドメイン内部]
    Check{不変条件}
    Check -- 違反 --> Throw[例外を投げる ⚡]
  end
  subgraph Boundary ["境界 (UI/API)"]
    Catch["catch 地獄... 😵‍💫"]
  end
  Throw --> Catch
```
...
```mermaid
flowchart LR
  subgraph Bad [ダメな例: throw ex;]
    A1[元々のエラー場所] -- リセット --> A2[throw ex した場所]
    A2 -- 報告 --> UserA[ここが原因に見える 😱]
  end
  subgraph Good [良い例: throw;]
    B1[元々のエラー場所] -- 維持 --> B2[throw; した場所]
    B2 -- 報告 --> UserB[真犯人の場所がわかる ✨]
  end
```
