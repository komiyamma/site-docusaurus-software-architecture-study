...
![Type Safe Box](./picture/invariants_cs_study_009_safebox.png)
...
flowchart TD
  subgraph Bug [取り違え事故]
    A[Email] -->|うっかり| F1[userName 引数へ]
    B[UserName] -->|うっかり| F2[email 引数へ]
  end
  F1 & F2 --> Run[実行できてしまう 😇]
```
...
```mermaid
flowchart LR
  subgraph Types [型で分ける]
    E[Email型]
    U[UserName型]
  end
  E -- X --> Func["Register(UserName, ...)"]
